...
/**
 * Commits one or more file changes to the repo as a single atomic commit,
 * so a Publish that edits several content files (e.g. a page's JSON plus a
 * newly-uploaded image) never lands as multiple partial commits.
 */
export async function publishFiles(files: FileChange[], message: string) {

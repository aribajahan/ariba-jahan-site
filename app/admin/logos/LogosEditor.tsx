...
export default function LogosEditor({ initialItems }: { initialItems: Logo[] }) {
  ...
  const addNew = () => {
    setItems((list) => [...list, { src: "", alt: "New Logo", heightPx: 44 }]);
    setOpenIndex(items.length);
  };

  const handlePublish = () =>
    publish(
      [{ path: "content/collections/speaking-logos.json", content: JSON.stringify(items, null, 2) + "\n" }],
      "Update Speaking Logos via Studio"
    );
  ...
  return (
    <div className="max-w-[900px] p-10">
      ...
      <div className="flex justify-between items-center mb-7">
        <h1 className="text-2xl">Speaking Logos</h1>
        <button
          type="button"
          onClick={addNew}
            ...
            {openIndex === i && (
              <div className="mt-3 pt-3 border-t border-[#f0efec] flex flex-col gap-2">
                ...
                <label className="flex items-center gap-[6px] text-[12px]">
                  <input type="checkbox" checked={!!logo.invert} onChange={(e) => update(i, { invert: e.target.checked })} />
                  Invert to white (for dark logos)
                </label>
      ...
      <div className="mt-5 text-[13px] text-[#999]">
        {items.length} total. Order determines which logos are the first 12 shown before the mobile/tablet &ldquo;Show more&rdquo; toggle.
      </div>

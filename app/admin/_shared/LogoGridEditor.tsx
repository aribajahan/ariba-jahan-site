...
export default function LogoGridEditor({
  initialItems,
  title,
  contextLabel,
  publishPath,
  ...
  const addNew = () => {
    setItems((list) => [...list, { src: "", alt: "New Logo", heightPx: defaultHeightPx }]);
    setOpenIndex(items.length);
  };

  const handlePublish = () => publish([{ path: publishPath, content: JSON.stringify(items, null, 2) + "\n" }], publishMessage);

  return (
    <div className="max-w-[900px] p-10">
      <div className="text-xs font-semibold text-[#888] mb-[6px]">{contextLabel}</div>
